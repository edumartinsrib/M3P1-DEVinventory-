...
class UserCreateSchema(Schema):
    city_id = fields.Integer(
        required=True, error_messages=set_error_message('city_id')
    )
    gender_id = fields.Integer(
        required=True, error_messages=set_error_message('gender_id')
    )
    role_id = fields.Integer(
        required=True, error_messages=set_error_message('role_id')
    )
    ...
    email = fields.Email(
        required=True, error_messages=set_error_message('email')
    )
    phone = fields.Str(error_messages=set_error_message('phone'))
    password = fields.Str(
        required=True, error_messages=set_error_message('password')
    )
    ...
    number_street = fields.Integer(
        error_messages=set_error_message('number_street')
    )
    ...
    def strong_password(self, value):
        if (
            ...
            raise ValidationError(
                'Senha fraca, utilize letras maiúsculas, minúsculas, números e caracteres especiais'
            )
