...
class InventoryCreateSchema(Schema):
    user_id = fields.Integer(
        Required=False, error_messages=set_error_message('user_id')
    )
    product_category_id = fields.Integer(
        required=True, error_messages=set_error_message('product_category_id')
    )
    title = fields.Str(
        required=True, error_messages=set_error_message('title')
    )
    value = fields.Float(
        required=True, error_messages=set_error_message('value')
    )
    brand = fields.Str(
        required=True, error_messages=set_error_message('brand')
    )
    template = fields.Str(
        required=True, error_messages=set_error_message('template')
    )
    description = fields.Str(
        required=True, error_messages=set_error_message('description')
    )
    product_code = fields.Integer(
        required=True, error_messages=set_error_message('product_code')
    )
