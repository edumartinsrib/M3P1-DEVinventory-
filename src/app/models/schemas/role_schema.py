...
class RoleValidateSchema(Schema):
    ...
    permissions = fields.List(fields.Integer(), required=True, error_messages=set_error_message('permissions'))
    ...
    def validate_permissions(self, value):
        for permission_id in value:
            if not Permission.query.filter_by(id=permission_id).first():
                raise ValidationError(f'{permission_id} - Permissão não encontrada')
